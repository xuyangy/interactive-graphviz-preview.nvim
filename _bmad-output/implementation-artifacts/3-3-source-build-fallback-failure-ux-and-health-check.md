...
- 2026-06-05: Post-release fix during v0.1.0 ship verification (retro Action Item 1). The real prebuilt install path crashed at checksum verification with `E976: Using a Blob as a String`: Neovim converts a Lua string containing NUL bytes into a Blob at the Lua/Vimscript boundary, and `vim.fn.sha256()` rejects a Blob — so it cannot hash a real binary at all. This was masked because `tests/install_spec.lua` stubbed `vim.fn.sha256` and no real release existed to download during dev (the AC2/AC3/AC9 prebuilt download→verify→promote path was never exercised on actual binary bytes). Fix: `install.lua` `digest_file` now computes SHA-256 by shelling out (list-form `vim.system`, no shell string) to `sha256sum`/`shasum`/`openssl` — whose hex digest matches the Node-crypto digests in `checksums.txt` — and fails closed if none is on PATH. This intentionally supersedes the Story 3.2 "do not shell out for hashing" guardrail, which rested on the false premise that `vim.fn.sha256` could hash binaries. Added a real-tool regression test (`digest_file hashes real binary bytes`) over NUL-containing content, and updated the install_spec stub harness to model the hasher. Verified end-to-end against the live `v0.1.0` release: a fresh `resolve_server_cmd()` downloaded `server-darwin-x64`, verified it, atomically promoted it, and `chmod 755`'d it, with the promoted binary's SHA matching the committed manifest. Validation: install_spec 14/14, health_spec 6/6 (shim under `lua`), `stylua --check .` clean, `git diff --check` clean, headless nvim smoke clean. No server/binary change → `checksums.txt` and the published `v0.1.0` assets remain valid (client-side hashing fix only; no re-release needed).
