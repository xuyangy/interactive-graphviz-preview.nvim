...
- The live DOM emphasis path in render.ts (~200 lines: `applyHighlightToDom`, `handleAppClick`, `nodeTitleFromClickTarget`, `extractModelFromApp`, `recomputeAndApplyHighlight`, `reapplyHighlightAfterRender`, `installInteractionHandlers`) has no automated test. This is the same project-wide limitation already recorded as "browser WASM render path untested" — the pure highlight math/selection/cluster derivation IS fully covered in interact.test.ts (33 cases). Verified manually in a browser per the Dev Agent Record. [frontend/render.ts:273-506] ✅ resolved in debt paydown 2026-06-10: render.dom.test.ts (20 cases, happy-dom) drives the exported handlers against a static graphviz-shaped SVG fixture — click delegation, multi-select, Esc, cluster augment, post-render reapply/prune, error/empty overlays, and the search box DOM. Residual: the WASM layout/render itself (d3-graphviz renderDot) still has no automated harness — a real-browser smoke remains the follow-up for that narrower gap.
