...
- [ ] Extend installer fallback while preserving Story 3.2 prebuilt behavior (AC: 1-6, 9)
  - [ ] Keep `resolve_server_cmd()` as the public contract used by `server.lua`; return a command list only after a binary is verified or source-built.
  - [ ] Refactor unsupported-platform detection so the prebuilt path returns structured fallback context instead of throwing before fallback can run.
  - [ ] Add Bun discovery/version helpers using list-form `vim.system()`; parse semver numerically and require local `bun --version` >= `1.3.10` unless implementation proves a higher minimum is necessary.
  - [ ] Build to a unique temp path under an ignored local directory, verify the output exists, then atomically promote to a fallback binary path.
  - [ ] Run build commands from the plugin root; do not depend on current working directory.
  - [ ] Ensure failed fallback builds remove temp files and do not overwrite a known-good fallback binary.
- [ ] Implement no-orphan-safe source-build process handling (AC: 4, 6)
  - [ ] Do not return `{ "bun", "run", "server/server.ts" }` as the runtime server command after fallback; return the compiled fallback executable path.
  - [ ] If a wrapper/process-group helper is needed for the build step, confine it to build time and ensure it is killed/cleaned on failure.
  - [ ] Preserve `server.lua`'s `vim.system(cmd, { stdin = true, ... })` spawn model and EOF heartbeat semantics.
- [ ] Implement user-facing failure UX (AC: 2, 3, 5)
  - [ ] Make unsupported-platform fallback copy explicit and discoverable through `vim.notify`/installer error text, not only debug logs.
  - [ ] Include platform, required Bun version, command attempted, and concise stdout/stderr when fallback fails.
  - [ ] Avoid noisy stack traces for normal missing-prerequisite cases.
- [ ] Implement Lua `:checkhealth` diagnostics (AC: 7, 9)
  - [ ] Replace scaffold-only `lua/interactive-graphviz/health.lua` with real health checks using `vim.health.start/ok/warn/error/info`.
  - [ ] Check Neovim >= 0.10.
  - [ ] Check committed `checksums.txt` can be found and parsed.
  - [ ] Check current mapped prebuilt binary presence and checksum where a prebuilt mapping exists.
  - [ ] Check fallback Bun executable and version without requiring Bun for supported prebuilt cache hits.
  - [ ] Check localhost port-bind capability using a short-lived loopback probe and release the port.
- [ ] Implement server `/health` endpoint (AC: 8)
  - [ ] Expand `server/health.ts` beyond scaffold status.
  - [ ] Route `GET /health` in `server/server.ts` without interfering with `/` static frontend or WebSocket upgrade.
  - [ ] Report pid, port, bind address, protocol/version, and session count from the existing server/session state.
  - [ ] Keep stdout reserved for JSON-line protocol only; HTTP health diagnostics must not write to stdout.
- [ ] Add focused tests (AC: 1-9)
  - [ ] Extend `tests/install_spec.lua` for unsupported platform fallback, missing Bun, old Bun, build failure cleanup, and successful fallback returning compiled executable.
  - [ ] Add `tests/health_spec.lua` or equivalent Lua tests for Neovim version, checksum, Bun, and port-bind reporting with stubs.
  - [ ] Add/extend `server/server.test.ts` or `server/health.test.ts` for `/health` JSON shape.
  - [ ] Keep existing installer tests green, especially cached prebuilt path no-download behavior.
